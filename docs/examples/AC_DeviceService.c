...
* History:
*************************************************/
int AC_QiniuPutFile(char *pLocalFilePath,char *uploadtoken)
{
    Qiniu_Client client;
    Qiniu_Error error;
    int ret = 0;
    ...
    error = Qiniu_Io_PutFile(&client, NULL, uploadtoken, NULL, pLocalFilePath, NULL);
    if (error.code != 200) 
    {
        ret = error.code;
    }
...
int AC_UploadFile(char *remotefilename,char *localfilename)
{
    ...
    {
        ret = AC_AwsPutFile(localfilename,g_struAcUploadInfo.chUploadToken);
    }
    else if(!strcmp(g_struAcUploadInfo.chStoreType,"qiniu"))
    {
        ret = AC_QiniuPutFile(localfilename,g_struAcUploadInfo.chUploadToken);
    }
    else
    {
        ret = -1;
